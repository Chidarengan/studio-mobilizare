...
function AdminPanel({ whatsappNumber, setWhatsappNumber, showInstagram, setShowInstagram }) {
  ...
  if (!isAdminOpen) {
    return (
      <button 
        ...
        className="fixed bottom-4 left-4 z-40 bg-[#1E2229] text-[#C5A880] p-3 rounded-full shadow-lg border border-[#C5A880]/20 text-xs"
      >
        Admin
      </button>
    );
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 bg-[#1E2229] text-white rounded-3xl p-5 shadow-2xl border border-[#C5A880]/30 max-w-xs">
        ...
        <div className="space-y-1">
          ...
          <div className="flex space-x-1">
            ...
            <button 
              ...
              className="bg-[#C5A880] text-[#1E2229] font-bold text-[10px] px-3 rounded"
            >
              Salvar
            </button>
          </div>
          {showSuccess && <p className="text-[9px] text-emerald-400">Atualizado!</p>}
        </div>
      </div>
    </div>
  );
